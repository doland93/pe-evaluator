...
def find_in_roster(raw_name, roster):
    ...
    for team, members in roster.items():
        for mb in members:
            ...
            # 2) 영문 완전/포함 일치
            if mb["english"]:
                eng = re.sub(r'\s+','', mb["english"])
                if raw_eng and (raw_eng == eng or raw_eng in eng or eng in raw_eng):
                    return team, mb["display"]
                if raw_eng_paren and eng.startswith(raw_eng_paren):
                    return team, mb["display"]
            # 유사도 (한글)
            ...
            # 유사도 (영문)
            if mb["english"] and raw_eng:
                eng = re.sub(r'\s+','', mb["english"])
                s = SequenceMatcher(None, raw_eng, eng).ratio()
                if s > best_score:
                    best_score = s; best_team = team; best_disp = mb["display"]
