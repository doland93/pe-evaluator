...
def find_in_roster(raw_name, roster):
    ...
    매칭 우선순위:
    1) 한글 완전일치
    2) 영문 단어 하나라도 명단 영문 이름에 포함 (순서 무관)
    3) 영문 전체 포함
    4) 유사도 매칭
    """
    ...
    # 영문 단어들 추출 (순서 무관 매칭용)
    raw_words = [w.upper() for w in re.findall(r'[A-Za-z]+', raw)]
    ...
    for team, members in roster.items():
        for mb in members:
            ...
            if mb["english"]:
                eng_words = [w.upper() for w in mb["english"].split()]
                eng_full = re.sub(r'\s+','', mb["english"].upper())

                # 2) 영문 단어 하나라도 일치 (순서 무관)
                # raw의 각 단어가 명단 영문 이름의 단어와 일치하는지
                for rw in raw_words:
                    if len(rw) >= 3 and rw in eng_words:
                        return team, mb["display"]

                # 3) 영문 전체 포함
                if raw_eng_full and (raw_eng_full == eng_full or
                                     raw_eng_full in eng_full or
                                     eng_full in raw_eng_full):
                    return team, mb["display"]

                # 유사도 (영문 단어별)
                for rw in raw_words:
                    if len(rw) < 3: continue
                    for ew in eng_words:
                        s = SequenceMatcher(None, rw, ew).ratio()
                        if s > best_score:
    ...
    if best_score >= 0.75:  # 유사도 기준 높임 (오매칭 방지)
...
def _parse_pdf(path):
    ...
    for line in text.split("\n"):
        ...
        # 한글 이름
        m = re.match(
            r"^([1-5])\s*[.。]\s*(?:\d{7,10}\s+)?([가-힣]{2,6})\s+(\d{1,3})\s*$",
            line)
        if m:
            result["scores_raw"].append((m.group(2).strip(), int(m.group(3))))
            continue
        # 영문 이름 (이름 성 또는 성 이름, 공백 포함)
        m = re.match(
            r"^([1-5])\s*[.。]\s*(?:\d{7,10}\s+)?([A-Za-z]+(?:\s+[A-Za-z]+)?)\s+(\d{1,3})\s*$",
            line)
        if m:
            result["scores_raw"].append((m.group(2).strip(), int(m.group(3))))
    return result
